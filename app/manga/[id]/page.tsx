...
import { isMHId, getMHMangaDetail } from '@/lib/mangahook';
...
export default async function MangaDetailPage({ params, searchParams }: Props) {
  ...
  if (isMHId(id)) {
    // ── MangaHook / Mangakakalot path ──────────────────────────
    const manga = await getMHMangaDetail(id);
    if (!manga) return notFound();

    const CHAPTERS_PER_PAGE = 96;
    const totalChapters = manga.chapterList.length;
    const totalPages = Math.ceil(totalChapters / CHAPTERS_PER_PAGE);
    const start = (page - 1) * CHAPTERS_PER_PAGE;
    const chapters = manga.chapterList.slice(start, start + CHAPTERS_PER_PAGE);
    const firstChapter = manga.chapterList[manga.chapterList.length - 1] ?? null; // MK lists newest first
    ...
    return (
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '1.5rem 1.5rem 4rem' }}>
        <nav style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '1.25rem' }}>
          ...
          <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.68rem', letterSpacing: '0.1em', color: 'var(--paper-2)', textTransform: 'uppercase' }}>{manga.name}</span>
        </nav>
        ...
        <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', marginBottom: '3rem' }}>
          <div style={{ flexShrink: 0, width: '200px' }}>
            <div style={{ position: 'relative', aspectRatio: '2/3', overflow: 'hidden', background: 'var(--surface)' }}>
              ...
              <img src={manga.imageUrl} alt={manga.name} referrerPolicy="no-referrer"
                style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
            {firstChapter && (
              <Link href={`/manga/${id}/${firstChapter.id}`}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
              <span style={{ padding: '3px 10px', background: 'var(--red)', fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.6rem', letterSpacing: '0.12em', color: 'var(--paper)', textTransform: 'uppercase' }}>MANGA</span>
              <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.6rem', letterSpacing: '0.12em', color: manga.status === 'Completed' ? 'var(--paper-2)' : 'var(--lime)', textTransform: 'uppercase' }}>● {manga.status.toUpperCase()}</span>
            </div>
            ...
            {manga.author && <p style={{ fontFamily: 'var(--font-body, sans-serif)', fontSize: '0.72rem', color: 'var(--muted)', marginBottom: '0.75rem' }}>by {manga.author}</p>}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '1rem' }}>
              {manga.genres.map(g => (
                <span key={g} style={{ padding: '2px 8px', border: '1px solid var(--border-2)', fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.55rem', letterSpacing: '0.1em', color: 'var(--muted)', textTransform: 'uppercase' }}>{g}</span>
              ))}
            </div>
            {manga.description && (
              <div>
                <p style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.62rem', letterSpacing: '0.1em', color: 'var(--red)', textTransform: 'uppercase', marginBottom: '0.35rem' }}>SYNOPSIS</p>
            ...
            <p style={{ marginTop: '1rem', fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.6rem', letterSpacing: '0.08em', color: 'var(--muted)', textTransform: 'uppercase' }}>
              {totalChapters} CHAPTERS · VIA MANGAKAKALOT
            </p>
          </div>
        </div>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem', marginBottom: '1rem' }}>
            <h2 style={{ fontFamily: 'var(--font-display, Impact)', fontSize: '1.4rem', letterSpacing: '0.08em', color: 'var(--paper)' }}>CHAPTERS</h2>
            <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.65rem', letterSpacing: '0.1em', color: 'var(--muted)', textTransform: 'uppercase' }}>{totalChapters} TOTAL</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', marginBottom: '1.5rem' }}>
            {chapters.map(ch => (
              <HoverLink key={ch.id} href={`/manga/${id}/${ch.id}`}
                ...
                <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.78rem', letterSpacing: '0.08em', color: 'var(--paper)', textTransform: 'uppercase' }}>
                  {ch.name}
                </span>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexShrink: 0 }}>
                  <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.55rem', color: 'var(--muted)', letterSpacing: '0.06em' }}>{ch.createdAt}</span>
                  <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.55rem', color: 'var(--red)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>READ ›</span>
                </div>
              </HoverLink>
            ))}
          </div>
          {totalPages > 1 && (
            <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                <Link key={p} href={`/manga/${id}?page=${p}`}
  ...
  const totalPages = Math.ceil(total / CHAPTERS_PER_PAGE);
    ...
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '1.5rem 1.5rem 4rem' }}>
      ...
      <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', marginBottom: '3rem' }}>
        ...
        <div style={{ flexShrink: 0, width: '200px' }}>
          <div style={{ position: 'relative', aspectRatio: '2/3', overflow: 'hidden', background: 'var(--surface)' }}>
            {manga.coverUrl ? (
              <Image src={manga.coverUrl} alt={manga.title} fill sizes="200px" style={{ objectFit: 'cover' }} priority />
            ) : (
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
            ...
            {manga.year && (
              <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.6rem', letterSpacing: '0.08em', color: 'var(--muted)' }}>{manga.year}</span>
            )}
          </div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '10px', borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem', marginBottom: '1rem' }}>
          <h2 style={{ fontFamily: 'var(--font-display, Impact)', fontSize: '1.4rem', letterSpacing: '0.08em', color: 'var(--paper)' }}>CHAPTERS</h2>
          <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.65rem', letterSpacing: '0.1em', color: 'var(--muted)', textTransform: 'uppercase' }}>{total} TOTAL</span>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', marginBottom: '1.5rem' }}>
          {chapters.map(ch => (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexShrink: 0 }}>
                <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.55rem', color: 'var(--muted)', letterSpacing: '0.06em' }}>{ch.scanlationGroup}</span>
                <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.55rem', color: 'var(--muted)', letterSpacing: '0.06em' }}>{ch.pages}P</span>
                <span style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.55rem', color: 'var(--red)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>READ ›</span>
              </div>
            </HoverLink>
          ))}
