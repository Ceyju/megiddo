...
export default function MangaCard({ manga, href, priority }: { manga: MDManga; href?: string; priority?: boolean }) {
  ...
  return (
    <Link href={resolvedHref} style={{ textDecoration: 'none', display: 'block' }}>
      ...
      <p style={{ fontFamily: 'var(--font-condensed, Arial)', fontSize: '0.72rem', letterSpacing: '0.05em', color: hovered ? 'var(--paper)' : 'var(--paper-2)', textTransform: 'uppercase', lineHeight: 1.2, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', transition: 'color 0.2s' }}>
        {manga.title}
      </p>
      {manga.latestChapter && (
